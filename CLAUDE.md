...
- **Imagens TCGdex localizadas são mentira** (verificado 2026-07-27): a API de locales como `pt` retorna bases `assets.tcgdex.net/pt/...` que **404am no CDN**. `TcgdexSource.imageBase()` normaliza toda URL de imagem para o caminho `en` (canônico, sempre presente) — obrigatório, pois URLs vão para notas Markdown sem fallback possível.
